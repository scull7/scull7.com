(* Isolated mail action. Smallest real path: HTTP to the existing Resend
   sender. Tests inject capture() so no inbox is required. *)
...
let unused () =
  {
    send =
      (fun _ ->
        return (Error "missing_env:RESEND_API_KEY"));
  }
