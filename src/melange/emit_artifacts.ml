...
let bundle_interview () =
  ...
  if status <> 0 then (
    if String.trim result##stderr <> "" then Js.Console.error result##stderr;
    if String.trim result##stdout <> "" then Js.Console.error result##stdout;
    Js.Console.error
      ("emit-artifacts: esbuild interview bundle failed ("
     ^ string_of_int status ^ ")");
    Node.Process.exit 1);
  if not (Node.Fs.existsSync interview_out) then (
    Js.Console.error ("emit-artifacts: esbuild did not write " ^ interview_out);
    Node.Process.exit 1);
...
let () =
  ...
  if not (Node.Fs.existsSync fn_src) then (
    Js.Console.error ("emit-artifacts: missing functions emit at " ^ fn_src);
    Node.Process.exit 1);
