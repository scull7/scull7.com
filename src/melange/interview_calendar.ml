(* Isolated Google Calendar action. create_tentative and delete_event are
   the only side effects. Tests inject capture() so no live GCal is used. *)
...
let service_account_jwt (cfg : Interview_config.t) =
  ...
  | Some email, Some pem ->
      let now = int_of_float (Interview_clock.now_ms () /. 1000.) in
      let header =
      ...
      in
      let unsigned =
        Interview_token.b64url_of_string header
        ^ "."
        ^ Interview_token.b64url_of_string claim
      in
      let jwt =
        unsigned ^ "."
        ^ Interview_token.rsa_sign_b64url (unescape_pem pem) unsigned
      in
      let headers = Js.Dict.empty () in
      Js.Dict.set headers "Content-Type" "application/x-www-form-urlencoded";
      let body =
...
let delete_event_http token calendar_id event_id =
  ...
  let cal = Interview_token.encode_uri calendar_id in
  let ev = Interview_token.encode_uri event_id in
  let url =
...
let create_event token (req : request) =
  let headers = Js.Dict.empty () in
  Js.Dict.set headers "Authorization" ("Bearer " ^ token);
  Js.Dict.set headers "Content-Type" "application/json";
  let cal = Interview_token.encode_uri req.calendar_id in
  let url =
