...
| Board | **Pinto is source of truth.** Existing `.pinto/` (key `T`). New IDs continue from `.pinto/issued_ids`. After rebase onto `main`, Phase 3 PBIs are T-20–T-25 (T-14/T-15 taken on `main`; T-16–T-19 reserved for in-flight interview-me). `pinto automate --plan … --dry-run --json` before multi-step writes. |
